//! @title FIR Filter
//! @file filtro_fir.v
//! @author Advance Digital Design - Ariel Pola
//! @date 29-08-2021
//! @version Unit02 - Modelo de Implementacion

//! - Fir filter with 4 coefficients 
//! - **i_srst** is the system reset.
//! - **i_en** controls the enable (1) of the FIR. The value (0) stops the systems without change of the current state of the FIR.
//! - Coefficients [-1, 1/2, -1/4, 1/8]

module filtro_fir
  #(
    parameter NB_INPUT   = 16, //! NB of input
    parameter NBF_INPUT  = 15, //! NBF of input
    parameter NB_OUTPUT  = 18, //! NB of output
    parameter NBF_OUTPUT = 17, //! NBF of output
    parameter NB_COEFF   = 16, //! NB of Coefficients
    parameter NBF_COEFF  = 15  //! NBF of Coefficients
  ) 
  (
    output signed [NB_OUTPUT-1:0] o_os_data, //! Output Sample
    input  signed [NB_INPUT -1:0] i_is_data, //! Input Sample
    input                         i_en     , //! Enable
    input                         i_srst   , //! Reset
    input                         clk        //! Clock
  );

  // NB_ADD  = 8 + 8 + 2 = 18.
  localparam NB_ADD     = NB_COEFF  + NB_INPUT + 2;
  // NBF_ADD = 7 + 7 = 14.
  localparam NBF_ADD    = NBF_COEFF + NBF_INPUT;
  // NBI_ADD = 18 + 14 = 32.
  localparam NBI_ADD    = NB_ADD    - NBF_ADD;
  // NBI_OUTPUT = 8 + 7 = 15.
  localparam NBI_OUTPUT = NB_OUTPUT - NBF_OUTPUT;
  // NB_SAT = 32 + 15 = 47.
  localparam NB_SAT     = (NBI_ADD) - (NBI_OUTPUT);

  //! Internal Signals.
  reg  signed [NB_INPUT         -1:0] register [3:1]; //! Matrix for registers
  wire signed [         NB_COEFF-1:0] coeff    [3:0]; //! Matrix for Coefficients
  wire signed [NB_INPUT+NB_COEFF-1:0] prod     [3:0]; //! Partial Products

  //! Coeff = [-1 1/2 -1/4 1/8]
  assign coeff[0] = 16'h8000; //h0 = -1
  assign coeff[1] = 16'h4000; //h1 = 1/2
  assign coeff[2] = 16'hE000; //h2 = -1/4
  assign coeff[3] = 16'h1000; //h3 = 1/8

  /*------- Shift Register ----------*/
  //////////////////////////////////////////
  // Option 1
  //////////////////////////////////////////
  
  //! ShiftRegister model
  always @(posedge clk) begin:shiftRegister
    // If synchronous reset is equal to 1.
    if (i_srst == 1'b1) begin
      // Reset registers.
      register[1] <= {NB_INPUT{1'b0}};
      register[2] <= {NB_INPUT{1'b0}};
      register[3] <= {NB_INPUT{1'b0}};
    end else begin
      // Do shift.
      if (i_en == 1'b1) begin
        register[1] <= i_is_data;
        register[2] <= register[1];
        register[3] <= register[2];
      end
    end
  end

  //////////////////////////////////////////
  // Option 2
  //////////////////////////////////////////

  //! ShiftRegister model
  // Using for loops.
  // They don't become hardware; they're only for describing repetitive
  // logic during synthesis.
  // Loop counters used in for loops.
  // integer ptr1;
  // integer ptr2;
  // always @(posedge clk) begin:shiftRegister // Label for hierarchy/debugging
  //   if (i_srst == 1'b1) begin
  //     for(ptr1=1;ptr1<4;ptr1=ptr1+1) begin:init
  //       register[ptr1] <= {NB_INPUT{1'b0}};
  //     end
  //   end else begin
  //     if (i_en == 1'b1) begin
  //       for(ptr2=1;ptr2<4;ptr2=ptr2+1) begin:srmove
  //         if(ptr2==1)
  //           register[ptr2] <= i_is_data;
  //         else
  //           register[ptr2] <= register[ptr2-1];
  //        end   
  //     end
  //   end
  // end

  /*------- Product ----------*/
  //////////////////////////////////////////
  // Option 1
  //////////////////////////////////////////
  //! Products
  assign prod[0] = coeff[0] * i_is_data;
  assign prod[1] = coeff[1] * register[1];
  assign prod[2] = coeff[2] * register[2];
  assign prod[3] = coeff[3] * register[3];

  //////////////////////////////////////////
  // Option 2
  //////////////////////////////////////////
  // This option execute lines as a template expansion.
  // It's not "running logic".
  // Generate multiple parallel multipliers using generate loop.
  // generate
  //   // It as a loop index used by the complier.
  //   genvar ptr;
  //   for(ptr=0;ptr<4;ptr=ptr+1) begin:mult
  //     if (ptr==0) 
  //       assign prod[ptr] = coeff[ptr] * i_is_data;
  //     else
  //       assign prod[ptr] = coeff[ptr] * register[ptr];
  //   end
  // endgenerate

  //////////////////////////////////////////
  // Option 3
  //////////////////////////////////////////
  // It multiplies (makes convolution) in each 
  // positive edge of clock.
  // Array created to storage the products x[n]*coeff[i].
  // reg signed [NB_INPUT+NB_COEFF-1:0] prod     [3:0]; //! Partial Products
  // generate
  //   genvar ptr;
  //   for(ptr=0;ptr<4;ptr=ptr+1) begin:mult
  //     if (ptr==0)
  //       always @(posedge clk) begin
  //         prod[ptr] <= coeff[ptr] * i_is_data;
  //       end 
  //     else
  //       always @(posedge clk) begin
  //         prod[ptr] <= coeff[ptr] * register[ptr];
  //       end 
  //   end
  // endgenerate

  //////////////////////////////////////////
  // Option 4
  //////////////////////////////////////////
  // This code describes FOUR PARALLEL, clocked multipliers that compute
  // FIR tap products and store them in registers on each
  // rising clock edge.
  // Makes convolution in positive edge of clock
  // or in any change signal of the system.
  // reg signed [NB_INPUT+NB_COEFF-1:0] prod     [3:0]; //! Partial Products
  // integer ptr;
  // always @(*) begin
  // The previous line create combinational logic.
  // prod would need to be wire, not reg.
  // You would lose the pipeline register.
  // Timing would get worse.
  // always @(posedge clk) begin// @(*) begin
  //   for(ptr=0;ptr<4;ptr=ptr+1) begin:mult
  //     if (ptr==0) 
  //       prod[ptr] <= coeff[ptr] * i_is_data;
  //     else
  //       prod[ptr] <= coeff[ptr] * register[ptr];
  //   end    
  // end

  /*------- Adders ----------*/
  //////////////////////////////////////////
  // Option 1
  //////////////////////////////////////////
  // This code is the final accumulation and output scaling stage of
  // a FIR Filter, including adder chain, bit-width reduction,
  // and signed saturation.
  //! Declaration
  wire signed [NB_ADD-1:0] sum      [3:1]; //! Add samples
  //! Adders
  assign sum[1] = prod[0] + prod[1];
  assign sum[2] = sum[1]  + prod[2];
  assign sum[3] = sum[2]  + prod[3];
  // Output
  // Output sample data.
  // Verify overflow (negative and positive saturation).
  // sum[3][18-1 -:47+1]?
  // -: downward part-select.
  // Select width bits, starting at start index, going downward (toward LSB).
  assign o_os_data = ( ~|sum[3][NB_ADD-1 -: NB_SAT+1] || &sum[3][NB_ADD-1 -: NB_SAT+1]) ? 
                      sum[3][NB_ADD-(NBI_ADD-NBI_OUTPUT) - 1 -: NB_OUTPUT] :
                      (sum[3][NB_ADD-1]) ? {{1'b1},{NB_OUTPUT-1{1'b0}}} : {{1'b0},{NB_OUTPUT-1{1'b1}}};
  // Are all bits zero? ~|
  // Are all bits one?  &

  // sum[3][NB_ADD-(NBI_ADD-NBI_OUTPUT) - 1 -: NB_OUTPUT]
  // sum[3][18-(32-15) -1 -: 8]= sum[3][0 -: 8]

  // (sum[3][NB_ADD-1])? sum[3][17] take sign
  // {{1'b1},{NB_OUTPUT-1{1'b0}}} : {{1'b0},{NB_OUTPUT-1{1'b1}}};
  // If sum[3] is positive output take maximum value.
  // else output minimum value.

  //////////////////////////////////////////
  // Option 2
  //////////////////////////////////////////
  //   integer ptr3;
  //   // [18-1:0]
  //   reg signed [NB_ADD-1:0] sum;
  //   always @(*) begin:accum
  //     // Repeat NB_ADD times 1'b0.
  //     sum = {NB_ADD{1'b0}};
  //     for(ptr3=0;ptr3<4;ptr3=ptr3+1) begin:adder 
  //       sum = sum + prod[ptr3];
  //     end
  //   end
  //  // Output
  //  assign o_os_data = ( ~|sum[NB_ADD-1 -: NB_SAT+1] || &sum[NB_ADD-1 -: NB_SAT+1]) 
  //                      ? 
  //                      sum[NB_ADD-(NBI_ADD-NBI_OUTPUT) - 1 -: NB_OUTPUT] :
  //                     (sum[NB_ADD-1]) ? {{1'b1},{NB_OUTPUT-1{1'b0}}} : {{1'b0},{NB_OUTPUT-1{1'b1}}};

endmodule
